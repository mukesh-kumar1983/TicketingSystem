 ...
 *     MainLayoutComponent
 *
 * This means changing the user's name in Settings immediately updates:
 *
 * - User display name.
 * - User avatar initial.
 * - Any other layout information based on CurrentUser.
 *
 * A logout/login cycle is therefore not required.
 * ============================================================================
 */
@Component({
...
export class MainLayoutComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.currentUserSubscription = this.authService.currentUser$.subscribe(
      (user) => {
         ...
         *
         * Angular will automatically update any template expressions that
         * depend on currentUser, userDisplayName, userInitial, or userRole.
         */
   ...
   *
   * The value is exposed as a property-style getter so the HTML template
   * can safely use:
   *
   *     *ngIf="isAdmin"
   *
   * Role comparison is case-insensitive to avoid UI inconsistencies caused
   * by differences such as "Admin" versus "admin".
   */
  get isAdmin(): boolean {
   ...
   *
   * This property is intentionally available for future role-specific
   * navigation or UI behaviour without changing the authentication model.
   */
   ...
   *
   * On desktop screens the sidebar is permanently visible.
   * On smaller screens this property controls whether the sidebar is open.
   */
  toggleSidebar(): void {
   ...
   *
   * This is called after selecting a navigation item or clicking the
   ...
   *
   * AuthService clears the locally stored authentication state and publishes
   * null through currentUser$, allowing all subscribed components to react
   * to the logout.
   */
  logout(): void {
    /**
   ...
   *
   * This value is displayed inside the user avatar.
   *
